...
| Evidence service | **PASS** | `services/evidence/service.go`, `cmd/evidence/main.go`, `services/evidence/internal/generator/generator_test.go`, `services/evidence/internal/generator/completeness_test.go`, `services/evidence/internal/generator/testdata/`, `services/evidence/internal/archive/archive_test.go`, `services/evidence/internal/export/export_test.go`, `services/evidence/internal/scheduler/scheduler_test.go` | On-demand and scheduled modes; generator, archiver, scheduler, and exporter all have tests. SOC2 control packs (CC6.1 access roster, CC6.2 privileged inventory, CC6.3 MFA coverage, CC7.2 admin activity, CC7.4 incident timelines) and ISO 27001:2022 packs (A.5.15 access control, A.5.26 incident management, A.8.15 logging/monitoring) are implemented. Schema assertion tests verify all required compliance-pack fields exist; golden-file tests (`testdata/expected_soc2_*.json/.csv`, `testdata/expected_iso27001_*.json/.csv`) verify structural completeness deterministically without a database. |
...
**RESOLVED** — SOC2 packs (CC6.1 access roster, CC6.2 privileged inventory, CC6.3 MFA coverage, CC7.2 admin activity, CC7.4 incident timelines) and ISO 27001:2022 packs (A.5.15 access control, A.5.26 incident management, A.8.15 logging/monitoring) are implemented. Schema assertion tests in `services/evidence/internal/generator/completeness_test.go` verify every required compliance-pack field is present in each artifact's JSON and CSV output. Golden-file tests (`testdata/expected_soc2_*.json/.csv`, `testdata/expected_iso27001_*.json/.csv`) provide deterministic structural regression — no database required. Run: `go test ./services/evidence/...`
...
| Evidence pack generation | `services/evidence/internal/generator/generator.go`, `services/evidence/internal/generator/completeness_test.go`, `services/evidence/internal/generator/testdata/`, `services/evidence/internal/packs/`, `docs/compliance/soc2-iso-pack.md` |
